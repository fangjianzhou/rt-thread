...
#define mmc_fpe_tx_interrupt_mask 0x8a4 
	...
	rt_pic_attach_irq(irq, rt_hw_gmac_isr, eqos, irq_name, RT_IRQ_F_NONE);
...
static int dwc_eqos_eth_init(struct eqos_eth_device *eqos)
{
	...
	rt_thread_t sp = rt_thread_create("speed",
                                    speed,
                                    RT_NULL,
                                    4096,
                                    1,
...
static int net_loopback(int argc, char **argv)
{
	struct eqos_eth_device *eqos = (struct eqos_eth_device *)rt_device_find("e0");
	setbits_le32(&eqos->mac_regs->configuration, EQOS_MAC_CONFIGURATION_LM);
	rt_hw_dsb(); 
    return 0;
}
MSH_CMD_EXPORT_ALIAS(net_loopback, net_loopback, print the last N - lines data of the given file);
struct dhcp_msg{
	char a[300];
};
static int send(int argc, char **argv)
{
	struct pbuf *p_out = pbuf_alloc(PBUF_TRANSPORT, sizeof(struct dhcp_msg), PBUF_RAM);
  	void* msg_out = p_out->payload;
  	memset(msg_out, 1, sizeof(struct dhcp_msg));
	rt_eqos_eth_tx(rt_device_find("e0"),p_out);
    return 0;
}
MSH_CMD_EXPORT_ALIAS(send, send, print the last N - lines data of the given file);
