...
static void sdhci_do_reset(struct sdhci_host *host, rt_uint8_t mask)
{
...
static void sdhci_write_block_pio(struct sdhci_host *host)
{
    ...
    while (blksize) {
        ...
        while (len) {
            scratch |= (rt_uint32_t)*buf << (chunk * 8);
...
static rt_uint8_t sdhci_calc_timeout(struct sdhci_host *host, struct mmc_command *cmd,
                 bool *too_big)
{
    rt_uint8_t count;
    struct mmc_data *data;
...
static int __sdhci_execute_tuning(struct sdhci_host *host, rt_uint32_t opcode)
{
    int i;
...
{
    rt_uint8_t mask = SDHCI_WAKE_ON_INSERT | SDHCI_WAKE_ON_REMOVE |
          SDHCI_WAKE_ON_INT;
    rt_uint32_t irq_val = 0;
    rt_uint8_t wake_val = 0;
    rt_uint8_t val;
...
bool sdhci_cqe_irq(struct sdhci_host *host, rt_uint32_t intmask, int *cmd_error,
           int *data_error)
{
    rt_uint32_t mask;
