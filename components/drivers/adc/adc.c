...
static rt_err_t _adc_control(rt_device_t dev, int cmd, void *args)
{
    ...
    {
        result = adc->ops->enabled(adc, (rt_uint8_t)(rt_ubase_t)args, RT_TRUE);
    }
    else if (cmd == RT_ADC_CMD_DISABLE && adc->ops->enabled)
    {
        result = adc->ops->enabled(adc, (rt_uint8_t)(rt_ubase_t)args, RT_FALSE);
    }
    ...
    else if (cmd == RT_ADC_CMD_GET_VREF && adc->ops->get_vrt_ref && args)
    {
        rt_int16_t value = adc->ops->get_vrt_ref(adc);
        if(value != 0)
        {
...
{
    rt_uint32_t value = 0;
    rt_int16_t vrt_ref = 0, voltage = 0;
    ...
    /*get the rt_reference voltage*/
    if (_adc_control((rt_device_t) dev, RT_ADC_CMD_GET_VREF, &vrt_ref) != RT_EOK)
    {
        goto _voltage_exit;
    }
    ...
    dev->ops->convert(dev, channel, &value);
    voltage = value * vrt_ref / (1 << resolution);
