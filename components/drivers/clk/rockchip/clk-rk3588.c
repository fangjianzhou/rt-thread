...
#if 0
struct rk3588_bigcore0cru
{
	struct rk3588_pll pll;   	/* 0x0000 */
	rt_uint32_t reserved0[152]; /* 0x0020 */
	rt_uint32_t mode_con00;		/* 0x0280 */
	rt_uint32_t reserved1[31]; 	/* 0x0284 */
	rt_uint32_t clksel_con[3];	/* 0x0300 */
	rt_uint32_t reserved1[317]; /* 0x030c */
	rt_uint32_t clkgate_con[2]; /* 0x0800 */
	rt_uint32_t reserved2[126]; /* 0x0808 */
	rt_uint32_t softrst_con[2]; /* 0x0a00 */
	rt_uint32_t reserved3[174]; /* 0x0a08 */
	rt_uint32_t divfree_con[2]; /* 0x0cc0 */
	rt_uint32_t reserved4[14];  /* 0x0cc8 */
	rt_uint32_t autoclk_con[2]; /* 0x0d00 */
};

struct rk3588_bigcore1cru
{
	rt_uint32_t reserved0[8];	/* 0x0000 */
	struct rk3588_pll pll;   	/* 0x0020 */
	rt_uint32_t reserved0[144]; /* 0x0040 */
	rt_uint32_t mode_con00;		/* 0x0280 */
	rt_uint32_t reserved1[31]; 	/* 0x0284 */
	rt_uint32_t clksel_con[3];	/* 0x0300 */
	rt_uint32_t reserved1[317]; /* 0x030c */
...
struct rk3588_dsucru
{
	rt_uint32_t reserved0[16];	/* 0x0000 */
	struct rk3588_pll pll;   	/* 0x0040 */
	rt_uint32_t reserved0[136]; /* 0x0060 */
	rt_uint32_t mode_con00;		/* 0x0280 */
	rt_uint32_t reserved1[31]; 	/* 0x0284 */
	rt_uint32_t clksel_con[8];	/* 0x0300 */
	rt_uint32_t reserved1[312]; /* 0x0320 */
	rt_uint32_t clkgate_con[4]; /* 0x0800 */
	rt_uint32_t reserved2[124]; /* 0x0810 */
	rt_uint32_t softrst_con[4]; /* 0x0a00 */
	rt_uint32_t reserved3[188]; /* 0x0a10 */
	rt_uint32_t autoclk_con[10];/* 0x0d00 */
	rt_uint32_t reserved4[118]; /* 0x0d28 */
	rt_uint32_t divfree_con[5]; /* 0x0f00 */
};

struct rk3588_phptopcru
{
	rt_uint32_t reserved0[128];	/* 0x0000 */
	struct rk3588_pll pll;   	/* 0x0200 */
	rt_uint32_t reserved0[376]; /* 0x0220 */
	rt_uint32_t clkgate_con;    /* 0x0800 */
	rt_uint32_t reserved1[127]; /* 0x0804 */
	rt_uint32_t softrst_con[8];	/* 0x0a00 */
};
#endif
struct rk3588_clk_priv {
	struct rk3588_cru *cru;
#if 0
    struct rk3588_bigcore0cru *bigcore0cru;
    struct rk3588_bigcore1cru *bigcore1cru;
    struct rk3588_dsucru *dsucru;
    struct rk3588_phptopcru *phptopcru;
...
//rt_kprintf("1110offset:%d enable con_reg vir %p-%p -phy %p\n",gate->offset,cru,con_regs,rt_kmem_v2p(con_regs));
...
static rt_err_t rk3588_clk_set_rate(struct rt_clk *clk, rt_ubase_t rate, rt_ubase_t prate)
{
	...
	case PLL_GPLL:
		...
		break;
	case PLL_NPLL:
		ret = rk_pll_set_rate(&rk3588_pll_clks[NPLL], priv->cru,
					    NPLL, rate);
		break;
	case PLL_V0PLL:
		ret = rk_pll_set_rate(&rk3588_pll_clks[V0PLL], priv->cru,
					    V0PLL, rate);
		priv->v0pll_hz = rk_pll_get_rate(&rk3588_pll_clks[V0PLL],
						       priv->cru, V0PLL);
		break;
	case PLL_AUPLL:
		ret = rk_pll_set_rate(&rk3588_pll_clks[AUPLL], priv->cru,
					    AUPLL, rate);
	...
	case HCLK_CENTER_ROOT:
	case ACLK_CENTER_LOW_ROOT:
		ret = rk3588_center_set_clk(priv, pdata->id, rate);
		break;
	case ACLK_TOP_ROOT:
	case PCLK_TOP_ROOT:
	case ACLK_LOW_TOP_ROOT:
		ret = rk3588_top_set_clk(priv, pdata->id, rate);
		break;
	case CLK_I2C0:
	case CLK_I2C1:
	case CLK_I2C2:
	...
	case CLK_I2C7:
	case CLK_I2C8:
		ret = rk3588_i2c_set_clk(priv, pdata->id, rate);
		break;
	case CLK_SPI0:
	case CLK_SPI1:
	case CLK_SPI2:
	case CLK_SPI3:
	case CLK_SPI4:
		ret = rk3588_spi_set_clk(priv, pdata->id, rate);
		break;
	case CLK_PWM1:
	case CLK_PWM2:
	case CLK_PWM3:
	case CLK_PMU1PWM:
		ret = rk3588_pwm_set_clk(priv, pdata->id, rate);
		break;
	case CLK_SARADC:
	case CLK_TSADC:
		ret = rk3588_adc_set_clk(priv, pdata->id, rate);
		break;
	case CCLK_SRC_SDIO:
	case CCLK_EMMC:
	case BCLK_EMMC:
	case SCLK_SFC:
	case DCLK_DECOM:
		ret = rk3588_mmc_set_clk(priv, pdata->id, rate);
		break;
	case TCLK_WDT0:
		ret = OSC_HZ;
		break;
	...
	case ACLK_VOP_LOW_ROOT:
	case HCLK_VOP_ROOT:
		ret = rk3588_aclk_vop_set_clk(priv, pdata->id, rate);
		break;
	case DCLK_VOP0:
	case DCLK_VOP0_SRC:
	case DCLK_VOP1:
	...
	case DCLK_VOP2_SRC:
	case DCLK_VOP3:
		ret = rk3588_dclk_vop_set_clk(priv, pdata->id, rate);
		break;
	case CLK_GMAC0_PTP_REF:
	case CLK_GMAC1_PTP_REF:
	case CLK_GMAC_125M:
	case CLK_GMAC_50M:
		ret = rk3588_gmac_set_clk(priv, pdata->id, rate);
		break;
	case SCLK_UART1:
	case SCLK_UART2:
	case SCLK_UART3:
	...
	case SCLK_UART8:
	case SCLK_UART9:
		ret = rk3588_uart_set_rate(priv, pdata->id, rate);
		break;
	case CLK_REF_PIPE_PHY0:
	case CLK_REF_PIPE_PHY1:
	case CLK_REF_PIPE_PHY2:
		ret = rk3588_pciephy_set_rate(priv, pdata->id, rate);
		break;
#endif
	default:
		return -RT_ENOENT;
