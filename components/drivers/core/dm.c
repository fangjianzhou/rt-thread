...
struct prt_refix_track
{
    rt_list_t list;

    int uid;
    const char *prt_refix;
};
static struct rt_spinlock _prt_refix_nodes_lock = { 0 };
static rt_list_t _prt_refix_nodes = RT_LIST_OBJECT_INIT(_prt_refix_nodes);

int rt_dm_dev_set_name_auto(rt_device_t dev, const char *prt_refix)
{
    int uid = -1;
    struct prt_refix_track *pt = RT_NULL;

    RT_ASSERT(dev != RT_NULL);
    RT_ASSERT(prt_refix != RT_NULL);

    RT_DEBUG_NOT_IN_INTERRUPT;

    rt_spin_lock(&_prt_refix_nodes_lock);

    rt_list_for_each_entry(pt, &_prt_refix_nodes, list)
    {
        /* caller always input constants string, check ptr is faster */
        if (pt->prt_refix == prt_refix || !rt_strcmp(pt->prt_refix, prt_refix))
        {
            uid = ++pt->uid;
            break;
        }
    ...
    rt_spin_unlock(&_prt_refix_nodes_lock);
    ...
    if (uid < 0)
    {
        ...
        pt->uid = uid = 0;
        pt->prt_refix = prt_refix;

        rt_spin_lock(&_prt_refix_nodes_lock);

        rt_list_insert_before(&_prt_refix_nodes, &pt->list);

        rt_spin_unlock(&_prt_refix_nodes_lock);
    }

    return rt_dm_dev_set_name(dev, "%s%u", prt_refix, uid);
}
