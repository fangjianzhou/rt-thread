...
{
    rt_err_t err;
    struct rt_ofw_node *np = pdev->parent.ofw_node, *shmem_np;
    ...
    if (!shmem_np || !rt_ofw_node_is_compatible(shmem_np, "arm,scmi-shmem"))
    {
        err = -RT_EINVAL;
    ...
    asmc->shmem = rt_ofw_iomap(shmem_np, 0);
    rt_ofw_node_put(shmem_np);
        ...
        struct rt_scmi_msg *msg)
{
    ...
    page = asmc->param_page;
    offset = asmc->param_offset;

    arm_smccc_smc(asmc->func_id, page, offset, 0, 0, 0, 0, 0, &res, RT_NULL);
