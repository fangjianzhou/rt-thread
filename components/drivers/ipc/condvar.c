                         ...
                         rt_tick_t timeout)
{
    ...
    if (!waiting_mtx)
        error = rt_atomic_compare_exchange_strong(&cv->waiting_mtx,
                                                  &waiting_mtx, (size_t)mtx);
    rt_spin_unlock(&_local_cv_queue_lock);

    if (error == 1 || waiting_mtx == (size_t)mtx)
    {
        rt_atomic_inc(&cv->waiters_cnt);
        ...
        rt_spin_lock(&_local_cv_queue_lock);
        if (rt_atomic_dec(&cv->waiters_cnt) == 1)
        {
            waiting_mtx = (size_t)mtx;
            error = rt_atomic_compare_exchange_strong(&cv->waiting_mtx,
                                                      &waiting_mtx, 0);
        ...
        error = rt_mutex_take(mtx, RT_WAITING_FOREVER);
        RT_ASSERT(error == 0);
    }
    else
    {
