...
rt_err_t rt_mbox_peek(struct rt_mbox_chan *chan)
{
    if (!chan)
    {
        return -RT_EINVAL;
...
rt_err_t rt_mbox_free(struct rt_mbox_chan *chan)
{
    if (chan)
    {
        chan->ctrl->ops->free(chan);
    }
    else
    {
        return -RT_EINVAL;
