...
static rt_uint8_t pci_find_next_cap_ttl(struct rt_pci_bus *bus, rt_uint32_t devfn, rt_uint8_t pos, int cap, int *ttl)
{
    rt_uint8_t ret = 0, id;
    rt_uint16_t ent;
...
static rt_uint8_t pci_find_next_cap(struct rt_pci_bus *bus, rt_uint32_t devfn, rt_uint8_t pos, int cap)
{
...
static rt_uint8_t pci_bus_find_cap_start(struct rt_pci_bus *bus, rt_uint32_t devfn, rt_uint8_t hdr_type)
{
    rt_uint8_t res = 0;
    rt_uint16_t status;
...
rt_uint8_t rt_pci_irq_intx(struct rt_pci_device *pdev, rt_uint8_t pin)
{
    ...
    if (!rt_pci_enabled(pdev, PCI_F_ARI))
    {
    ...
    for (int i = 0; i < host_bridge->bus_regions_nr; ++i)
    {
        ...
        LOG_I("Bus %s region(%d):",
            region->flags == PCI_BUS_REGION_F_MEM ? "Memory" :
                    (region->flags & PCI_BUS_REGION_F_PREFETCH ? "Prt_refetchable Mem" :
        ...
        void **out_addr, rt_size_t size, rt_ubase_t flags, rt_bool_t mem64)
{
    ...
    for (int i = 0; i < host_bridge->bus_regions_nr; ++i)
    {
        if (host_bridge->bus_regions[i].flags == flags && host_bridge->bus_regions[i].size > 0)
        {
            void *addr;
            region = &host_bridge->bus_regions[i];
            addr = (void *)(((region->bus_start - 1) | (size - 1)) + 1);

            if ((rt_uint64_t)addr - region->phy_addr + size <= region->size)
            {
                if (((rt_uint32_t)((((rt_uint64_t)addr) >> 16) >> 16)) && !mem64)
                {
                    region = RT_NULL;
                }
                else
...
rt_err_t rt_pci_device_alloc_resource(struct rt_pci_host_bridge *host_bridge, struct rt_pci_device *pdev)
{
    ...
    rt_size_t bars_nr;
    rt_uint8_t hdr_type;
    rt_bool_t prt_refetch = RT_FALSE;
    ...
    for (int i = 0; i < bars_nr; ++i)
    {
        ...
        else
        {
            ...
            if (prt_refetch && (cfg & PCIM_BAR_MEM_PREFETCH))
            {
...
rt_err_t rt_pci_device_register(struct rt_pci_device *pdev)
{
    ...
    return rt_bus_add_device(&pci_bus, &pdev->parent);
}
