...
#define SRAM_INIT_DONE(reg)                 (reg & RT_BIT(14))
...
struct rockchip_pcie3_phy
{
    ...
    void *regs;
    const struct rockchip_pcie3_phy_ops *ops;
...
struct rockchip_pcie3_phy_ops
{
    int (*phy_init)(struct rockchip_pcie3_phy *priv);
};
...
//                        0, 500);
//     if (ret)
//         pr_err("%s: lock failed 0x%x, check input rt_refclk and power supply\n",
//                __func__, reg);
//     return ret;
// }

static const struct rockchip_pcie3_phy_ops rk3568_ops =
{
...
static const struct rockchip_pcie3_phy_ops rk3588_ops =
{
...
RT_PLATFORM_DRIVER_EXPORT(rockchip_pcie3_phy_driver);
