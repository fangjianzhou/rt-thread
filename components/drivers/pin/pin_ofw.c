...
static char *ofw_pin_name_by_suffix(const char *propname, char *buffer, rt_size_t max_len)
{
    if (!propname)
    {
        return "gpios";
    ...
    {
        rt_snprintf(buffer, max_len, "%s-gpios", propname);
        ...
        rt_uint8_t *out_mode, rt_uint8_t *out_value)
{
    ...
    propname = ofw_pin_name_by_suffix(propname, gpios_name, sizeof(gpios_name));

    pin = rt_ofw_parse_phandle_cells(np, propname, "#gpio-cells", index, &pin_args);
...
rt_ssize_t rt_ofw_get_named_pin_count(struct rt_ofw_node *np, const char *propname)
{
    ...
    return rt_ofw_count_phandle_cells(np, propname, "#gpio-cells");
}