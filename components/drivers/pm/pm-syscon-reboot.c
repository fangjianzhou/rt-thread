...
#define DBG_TAG "pm.syscon.reboot"
...
static rt_err_t syscon_reboot_probe(struct rt_platform_device *pdev)
{
    ...
    {
        LOG_E("rt_pm_machine_reset have hook %p", rt_pm_machine_reset);

        return -RT_EBUSY;
    }

    rt_pm_machine_reset = syscon_reboot;
...
{
    .name = "pm-syscon-reboot",
    .ids = syscon_reboot_ofw_ids,

    .probe = syscon_reboot_probe,
};
RT_PLATFORM_DRIVER_EXPORT(syscon_reboot_driver);
