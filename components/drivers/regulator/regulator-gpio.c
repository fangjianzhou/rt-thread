...
static rt_err_t regulator_gpio_enable(struct rt_regulator_node *reg_np)
{
    ...
    if (rg->enable_pin < 0 || param->always_on)
    {
        return RT_EOK;
    }
    ...
    if (rg->enable_cnt == 0)
    {
        ...
        rt_hw_us_delay(param->enable_delay);
    }
    ++rg->enable_cnt;
...
static rt_bool_t regulator_gpio_is_enabled(struct rt_regulator_node *reg_np)
{
    ...
    active = rt_pin_read(rg->enable_pin);
        ...
        int min_uvolt, int max_uvolt)
{
    ...
    for (int i = 0; i < rg->pins_nr; ++i)
    {
        ...
        if (!rg->param.enable_active_high)
        {
            state = !state;
        }

        rt_pin_write(rg->pins_desc[i].pin, state);
    }
...
static rt_err_t regulator_gpio_probe(struct rt_platform_device *pdev)
{
    ...
    rg->enable_pin = rt_pin_get_named_pin(dev, "enable-gpios", 0, RT_NULL, RT_NULL);
    ...
    if (rg->pins_nr > 0)
    {
        ...
        for (int i = 0; i < rg->pins_nr; ++i)
        {
            ...
            gpiod->pin = rt_pin_get_named_pin(dev, "gpios", i, RT_NULL, RT_NULL);
    ...
    for (int i = 0; i < rg->states_nr; ++i)
    {
        if (rt_dm_dev_prop_read_u32_index(dev, "states", i * 2, &rg->states[i].value) < 0)
        {
            err = -RT_EIO;
        ...
        if (rt_dm_dev_prop_read_u32_index(dev, "states", i * 2 + 1, &rg->states[i].gpios) < 0)
        {
            err = -RT_EIO;
...
INIT_SUBSYS_LATER_EXPORT(regulator_gpio_register);
