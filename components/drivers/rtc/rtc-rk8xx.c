...
static void rk8xx_rtc_thread_isr(void *param)
{
    rt_err_t err;
    ...
    while (RT_TRUE)
    {
        rt_thread_suspend(rk8xx_rtc->irq_thread);
        rt_schedule();
...
static void rk8xx_rtc_isr(int irqno, void *param)
{
    ...
    rt_thread_resume(rk8xx_rtc->irq_thread);
}
...
static rt_err_t rk8xx_rtc_probe(struct rt_platform_device *pdev)
{
    ...
    rk8xx_rtc->irq_thread = rt_thread_create("rtc-rk8xx", &rk8xx_rtc_thread_isr,
            rk8xx_rtc, SYSTEM_THREAD_STACK_SIZE, RT_THREAD_PRIORITY_MAX / 2, 10);

    if (!rk8xx_rtc->irq_thread)
    {
        err = -RT_ERROR;
    ...
    rt_thread_startup(rk8xx_rtc->irq_thread);
    ...
    rt_hw_interrupt_install(rk8xx->irq, rk8xx_rtc_isr, rk8xx_rtc, "rtc-rk8xx");
...
_fail:
    if (rk8xx_rtc->irq_thread)
    {
        rt_thread_delete(rk8xx_rtc->irq_thread);
    }
    rt_free(rk8xx_rtc);
...
static int rk8xx_rtc_drv_register(void)
{
    rt_platform_driver_register(&rk8xx_rtc_driver);

    return 0;
}
INIT_DRIVER_LATE_EXPORT(rk8xx_rtc_drv_register);
