...
rt_inline void sdio_dw_set_cto(struct sdio_dw *sd)
{
     ...
     * cart_reful about synchronization here.  Specifically in hardware a command
...
static void sdio_dw_init_dma(struct sdio_dw *sd)
{
    ...
    */
    ...
    if (sd->use_dma == TRANS_MODE_IDMAC)
    {
        ...
        sd->dma_buf_cache = rt_pages_alloc(rt_page_bits(DESC_RING_BUF_SZ));
        ...
        sd->dma_buf_phy = (rt_ubase_t)rt_kmem_v2p(sd->dma_buf_cache);
        sd->dma_buf = rt_ioremap_nocache((void *)sd->dma_buf_phy, DESC_RING_BUF_SZ);

        if (!sd->dma_buf)
        {
            rt_pages_free(sd->dma_buf_cache, rt_page_bits(DESC_RING_BUF_SZ));

            LOG_E("Could not alloc DMA memory");
...
rt_err_t sdio_dw_probe(struct sdio_dw *sd)
{
    ...
    if (rt_dm_dev_prop_read_bool(&sd->parent, "resets"))
    {
        if (!(sd->rstc = rt_reset_control_get_by_name(&sd->parent, "reset")))
        {
...
_out_clk:
    ...
    rt_pin_ctrl_confs_apply_by_name(&sd->parent, RT_NULL);
    ...
    sd->state_wq = rt_workqueue_create(dev_name, RT_SYSTEM_WORKQUEUE_STACKSIZE,
            rt_min(RT_SDIO_THREAD_PRIORITY, RT_MMCSD_THREAD_PREORITY));
...
rt_err_t sdio_dw_remove(struct sdio_dw *sd)
{
    ...
    rt_hw_interrupt_uninstall(sd->irq, sdio_dw_isr, sd);
