...
#define DBG_TAG "sensor.rk-tsadc"
...
{
    struct rt_sensor_device parent;
...
static rt_ssize_t rk_tsadc_sensor_fetch_data(rt_sensor_t sensor,
        rt_sensor_data_t buf, rt_size_t len)
{
    struct rockchip_tsadc *rk_tsadc;
    const struct rockchip_tsadc_chip *chip;
    struct rockchip_tsadc_channel *chn = raw_to_rockchip_tsadc_channel(sensor);

    if (sensor->info.type == RT_SENSOR_TYPE_TEMP)
    {
        int temp;

        rk_tsadc = chn->rk_tsadc;
        chip = rk_tsadc->chip;

        if (!chip->get_temp(&chip->table, chn->id, rk_tsadc->regs, &temp))
        {
            buf->type = RT_SENSOR_TYPE_TEMP;
            buf->data.temp = temp / TEMP_SCALE;
            buf->timestamp = rt_sensor_get_ts();
...
static rt_err_t rk_tsadc_sensor_control(rt_sensor_t sensor, int cmd, void *arg)
{
    struct rockchip_tsadc_channel *chn = raw_to_rockchip_tsadc_channel(sensor);
...
const static struct rt_sensor_ops rk_tsadc_sensor_ops =
{
    .fetch_data = rk_tsadc_sensor_fetch_data,
    .control = rk_tsadc_sensor_control
};
...
static void rk_tsadc_isr(int irqno, void *param)
{
    ...
    {
        struct rockchip_tsadc_channel *chn = &rk_tsadc->channels[i];

        if (chn->parent.irq_handle)
        {
            chn->parent.irq_handle(&chn->parent);
        }
        else if (DBG_LEVEL >= DBG_ERROR)
        {
            /* shutdown if read temperature fail */
            int temp = rk_tsadc->tshut_temp;

            chip->get_temp(&chip->table, chn->id,
                    rk_tsadc->regs, &temp);

            LOG_E("Channel[%d]<%s> temperature = %d%s", i, chip->chn_name[i],
                    temp / TEMP_SCALE, RT_SENSOR_UNIT_CELSIUS_STR);

            if (temp >= rk_tsadc->tshut_temp)
            {
                rt_hw_cpu_shutdown();
            }
        }
...
static rt_err_t rockchip_tsadc_probe(struct rt_platform_device *pdev)
{
    ...
    if (!rk_tsadc->clk || !rk_tsadc->pclk ||
        rt_clk_enable(rk_tsadc->clk) || rt_clk_enable(rk_tsadc->pclk))
    {
        err = -RT_EIO;
    ...
    for (int i = 0; i < rk_tsadc->chip->chn_num; ++i)
    {
        ...
        struct rt_sensor_device *sensor = &channel->parent;

        channel->id = chip->chn_offset + i;
        ...
        sensor->info.name = "rockchip-tsadc";
        sensor->info.unit = RT_SENSOR_UNIT_CELSIUS;
        sensor->info.scale.range_max = table->id[0].temp / TEMP_SCALE;
        sensor->info.scale.range_min = table->id[table->length - 1].temp / TEMP_SCALE;
        sensor->info.acquire_min = 100;

        sensor->ops = &rk_tsadc_sensor_ops;

        rt_dm_dev_set_name_auto(&sensor->parent, "tsadc");
        dev_name = rt_dm_dev_get_name(&sensor->parent);

        rt_hw_sensor_register(sensor, dev_name, RT_DEVICE_FLAG_RDWR, RT_NULL);
    }
    ...
    for (int i = 0; i < rk_tsadc->chip->chn_num; ++i)
    {
        int tshut_temp = rk_tsadc->tshut_temp;
        struct rockchip_tsadc_channel *channel = &rk_tsadc->channels[i];

        tshut_temp -= tshut_temp / 4;

        if (!chip->set_alarm_temp(table, channel->id, rk_tsadc->regs, tshut_temp))
        {
            LOG_I("Channel[%d]<%s> set trip alarm %d%s ~ %d%s", i, chip->chn_name[i],
                    tshut_temp / TEMP_SCALE, RT_SENSOR_UNIT_CELSIUS_STR,
                    rk_tsadc->tshut_temp / TEMP_SCALE, RT_SENSOR_UNIT_CELSIUS_STR);
        }
    }

    rt_dm_dev_bind_fwdata(dev, RT_NULL, &rk_tsadc->parent);
