...
static void pci_serial_remove(struct serial8250 *serial)
{
    struct pci_serial *pci_serial = to_pci_serial(serial);

    rt_pci_intx(pci_serial->pci_dev, RT_FALSE);
...
static rt_err_t pci_serial_probe(struct rt_pci_device *pdev)
{
    ...
    // if (pdev->bus->cur_bus_speed == PCI_SPEED_UNKNOWN)
    // {
        serial->freq = 1843200;
    // }
    // else
    ...
    serial->remove = &pci_serial_remove;
    ...
    rt_pci_intx(pci_serial->pci_dev, RT_TRUE);
...
static struct rt_pci_device_id pci_serial_pci_ids[] =
{
