...
#define VIRTIO_CONSOLE_QUEUE_PORT0_RX   0
#define VIRTIO_CONSOLE_QUEUE_PORT0_TX   1
#define VIRTIO_CONSOLE_QUEUE_CTRL_RX    2
#define VIRTIO_CONSOLE_QUEUE_CTRL_TX    3

struct virtio_console_request
{
    /*
     * Layout:
     * +------------------------+
...
struct virtio_console
{
    ...
    struct virtio_console_request *request;
};
...
static rt_ssize_t virtio_port_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    return size;
}

static rt_ssize_t virtio_port_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    return size;
}

static rt_err_t virtio_port_control(rt_device_t dev, int cmd, void *args)
{
    rt_err_t err = RT_EOK;
...
const static struct rt_device_ops virtio_port_ops =
{
    .read = virtio_port_read,
    .write = virtio_port_write,
    .control = virtio_port_control,
};
#endif

static void virtio_console_done(struct rt_virtqueue *vq)
{
    struct virtio_console_request *request;
...
//     vconsole->virtq_nr = min(RT_CPUS_NR * 16, 1024);
//     vconsole->request_nr = vconsole->virtq_nr;
//     vconsole->request = rt_malloc(sizeof(struct virtio_console_request) * vconsole->request_nr);
...
//     {
//         rt_virtio_virtqueue_preset_max(&vconsole->vqs[i], vconsole->virtq_nr);
//     }

//     return rt_virtio_virtqueue_install(vconsole->vdev, RT_ARRAY_SIZE(names), vconsole->vqs, names, cbs);
// }
...
{
    rt_err_t err;
    struct virtio_port *vport;
    struct virtio_console *vconsole = rt_calloc(1, sizeof(*vconsole) + sizeof(*vport));
    ...
    vport = (void *)&vconsole[1];
    vconsole->vdev = vdev;

    rt_virtio_read_config(vconsole->vdev, struct virtio_console_config, max_nr_ports, &vconsole->max_port_nr);

    // err = virtio_console_vq_init(vconsole);
    ...
    vport->parent.type = RT_Device_Class_Char;
#ifdef RT_USING_DEVICE_OPS
    vport->parent.ops = &virtio_port_ops;
#else
    vport->parent.read = virtio_port_read;
    vport->parent.write = virtio_port_write;
    vport->parent.control = virtio_port_control;
#endif
