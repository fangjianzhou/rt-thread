...
#ifdef RT_USING_PM
static int dw_wdt_pm_suspend(const struct rt_device *device, rt_uint8_t mode)
{
...
static void dw_wdt_isr(int irqno, void *param)
{
    ...
    rt_event_send(&wdt->event, HWREG32(wdt->base + WDOG_INTERRUPT_CLEAR_REG_OFFSET));
}
...
static rt_err_t dw_wdt_probe(struct rt_platform_device *pdev)
{
    ...
    rt_event_init(&dw_wdt->event, dev_name, RT_IPC_FLAG_PRIO);
