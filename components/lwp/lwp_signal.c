...
rt_err_t lwp_signal_init(struct lwp_signal *sig)
{
    ...
    rc = timer_create(CLOCK_REALTIME_ALARM, &sevp, &sig->real_timer);
    if (rc != 0)
    {
        rc = GET_ERRNO();
    }
        ...
        if ((thread->stat & RT_THREAD_SUSPEND_MASK) == RT_THREAD_SUSPEND_MASK)
        {
            if ((thread->stat & RT_SIGNAL_COMMON_WAKEUP_MASK) != RT_SIGNAL_COMMON_WAKEUP_MASK)
            {
                ...
                need_schedule = 1;
                    ...
                    ((thread->stat & RT_SIGNAL_KILL_WAKEUP_MASK) != RT_SIGNAL_KILL_WAKEUP_MASK))
            {
                thread->error = RT_EINTR;
                rt_thread_wakeup(thread);
                need_schedule = 1;
