...
static int lwp_ctty_register(lwp_tty_t terminal)
{
    rt_err_t rc = -RT_ENOMEM;
    const char *tty_name = "tty";

    device_setup(terminal);
    rc = rt_device_register(&terminal->parent, tty_name, RT_DEVICE_FLAG_DYNAMIC);
    if (rc == RT_EOK)
    {
        terminal->parent.readlink = &ctty_readlink;
        terminal->parent.fops = &ctty_file_ops;
    }
    return rc;
}

static struct lwp_tty ctty;
