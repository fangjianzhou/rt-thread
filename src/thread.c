...
rt_err_t rt_thread_sleep(rt_tick_t tick)
{
    ...
    level_lock = rt_spin_lock_irqsave(&(thread->spinlock));
    ...
    if (err == RT_EOK)
    {
        ...
        /* enable interrupt */
        rt_spin_unlock_irqrestore(&(thread->spinlock), level_lock);
...
rt_err_t rt_thread_delay_until(rt_tick_t *tick, rt_tick_t inc_tick)
{
    ...
    /* disable interrupt */
    level = rt_hw_local_irq_disable();
    level_lock = rt_raw_spin_lock_irqsave(&(thread->spinlock));
    ...
    if (cur_tick - *tick < inc_tick)
    {
        ...
        *tick += inc_tick;
        ...
        rt_raw_spin_unlock_irqrestore(&(thread->spinlock), level_lock);
        /* suspend thread */
...
rt_err_t rt_thread_resume(rt_thread_t thread)
{
    ...
    level = rt_raw_spin_lock_irqsave(&(thread->spinlock)); //TODO need lock for cpu
...
rt_err_t rt_thread_wakeup(rt_thread_t thread)
{
    ...
    RT_ASSERT(thread != RT_NULL);
    RT_ASSERT(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread);
    temp = rt_raw_spin_lock_irqsave(&(thread->spinlock));
    func = thread->wakeup.func;
    thread->wakeup.func = RT_NULL;
    rt_raw_spin_unlock_irqrestore(&(thread->spinlock), temp);
    if (func)
    {
